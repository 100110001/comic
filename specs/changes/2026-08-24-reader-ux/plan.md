...
在桌面阅读形态上实现键盘翻页、沉浸式隐藏工具栏与图片失败重试，移动端行为保持不变，不触碰接口与数据层。改动集中在阅读器一个模块。

## Problem Frame

现有桌面阅读只有滚轮翻页，键盘与触控板用户缺少快捷路径；工具栏常驻分散注意力；图片失败后只有静态占位与空态，没有恢复入口。三项都是纯前端体验缺口，无数据层依赖。
...
- **沉浸隐藏只影响工具栏可见性，不改变布局与阅读状态**：无操作计时器驱动 AppBar 与进度条淡出，点击阅读区或翻页键恢复；滚轮翻页视为阅读动作，不触发恢复。隐藏期间阅读位置、滚轮与键盘行为保持可用。
...
- **Dependencies:** 无
- **Files:** `lib/screens/reader_screen.dart`
- **Approach:** 用无操作计时器驱动 AppBar 与底部进度条的淡出/恢复；点击阅读区与翻页键触发恢复并重置计时，滚轮翻页不触发；指针悬停于可见工具栏时保持显示；计时器随页面销毁清理。
...
- `specs/reader.spec.md` — updated：Public Contract 增加桌面键盘翻页、沉浸工具栏与图片失败重试的行为；Notes 补充"失败可点击重试"的设计意图。
