...
为桌面阅读形态增加键盘翻页、沉浸式自动隐藏工具栏与图片失败重试。移动端行为不变，仅前端改动。

## Problem Frame

当前桌面阅读只有滚轮一种翻页方式，触控板与键盘用户缺少快捷路径；AppBar 与进度条常驻，长读时分散注意力；单图或整章加载失败后只有静态占位与空态，没有恢复入口。这些都属于"每天都在用、每次都会感受到"的体验缺口，且不需要改动数据层。
