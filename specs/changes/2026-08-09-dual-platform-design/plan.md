...
- R1. 应用提供桌面与手机两套界面：Android/iOS 恒为手机形态；Windows/Linux/macOS 与 Web 按窗口宽度选择（宽屏桌面、窄屏手机）。（U2）
...
- **平台判定集中在一处：** Android/iOS 恒为手机；Windows/Linux/macOS 与 Web 按窗口宽度（≥720px 桌面，否则手机）选择导航与阅读器形态，宽度变化实时切换。页面内部不散落平台分支。
...
- 继续阅读跳转的进一步打磨（当前已做多帧重试修复；若仍不满足，单独走 define 解决）
...
- 平台判定是全局开关：外壳层按窗口宽度决定导航与阅读器形态，页面不散落平台分支；Android/iOS 恒为手机，其余平台宽窄窗口实时切换。
