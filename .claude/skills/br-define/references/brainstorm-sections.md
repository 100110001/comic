...
the rendering reference (`markdown-rendering.md`).

## The outcome

A great brainstorm produces a doc that enables three audiences to act:

- **The planning agent** (`br-plan` or a human) produces an implementation
...
- Any durable decisions made during the dialogue can flow naturally to
  downstream artifacts (`br-plan` or the commit message) without a
  definition doc as an intermediary.
...
decision flows to `br-plan` (or directly to the commit message) without a
definition doc in the middle.
...
- **`date`** — creation date in ISO 8601 (`YYYY-MM-DD`), ASCII digits only.
  Used in the change's working directory name (`specs/changes/YYYY-MM-DD-<slug>/`).
- **`topic`** — kebab-case slug identifying the change (e.g.,
  `rbac-outfitter-members`, `partner-payouts`). Used in the working directory
  name alongside `date`, and as the resume-detection key when `br-define`'s
  Phase 0.1 scans `specs/changes/` for an existing `define.md` to continue.
...
(`br-plan`) reference via the plan's `origin:` field. No artifact carries a
mutable status; whether work shipped is derived from git, not stored in the
doc. Do not introduce one.
...
- **No process exhaust.** No "captured at Phase X" notes, no `## Next Steps`
  pointing to br-plan, no italic provenance lines. Engineering process
...
This reference (`brainstorm-sections.md`) is about WHAT the definition doc
contains; the rendering reference is about HOW it is presented.
