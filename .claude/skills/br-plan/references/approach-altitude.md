...
This is not a software plan template (no implementation units / test scenarios) unless the deliverable itself is a software implementation plan — in which case "execute now / code" routes into the normal `br-plan` flow (below) rather than composing the deliverable here.
...
**Save for later.** Persist the approach-plan to `specs/changes/` so it survives. If the deliverable is non-code, write the marker (`execution: knowledge-work`, see `references/plan-sections.md`) at persist time — so a later `br-work` invocation on the saved plan routes to the carve-out, not the code path. Offer to deepen it. Keep the plan **agent-agnostic** (no `br-work`-specific choreography in the body) so any agent can execute it later.

**Execute now -- code deliverable.** The approach-plan's job is done; continue into the normal `br-plan` flow (Phase 0.1b onward) to produce the implementation plan, then hand off to `br-work` for the code. `br-plan` never writes the code itself.

**Execute now -- non-code deliverable.** This is the knowledge-work path with no `br-work` equivalent, so it routes to `br-work`'s carve-out:

1. Write the marker `execution: knowledge-work` into the plan frontmatter.
2. **Persist** the marked plan to `specs/changes/` (the marker needs a file to live in so it can travel — R7's file-optional governs the user keeping a chat-only copy, but non-code *execution* forces a persist).
3. Fire the `br-work` skill, passing the plan path, via the platform's skill-invocation primitive (`Skill` in Claude Code). Do not merely tell the user to run it — fire it so execution happens in this session.

`br-plan` itself does not execute the deliverable in any path — it produces the approach-plan and hands off. The portable plan is also runnable by any other agent without `br-work`.
