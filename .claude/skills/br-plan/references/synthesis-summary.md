...
This content is loaded when a synthesis-summary phase fires in br-plan. There are two variants — they share structure but differ in timing and content focus:
...
- AND Phase 0.4 stayed in br-plan (did not route to ce-debug, br-work, or universal-planning)
...
When the skill is invoked from an automated workflow such as LFG or any `disable-model-invocation` context, the skill runs in non-interactive mode (no synchronous user). The artifact is read by downstream skills (br-work) and human reviewers (PR review).
...
This restores the audit visibility the original design intended (un-validated bets must not propagate as authoritative content), but surfaces them under their own label rather than hiding them. Downstream review (br-work, human PR review) can scrutinize Assumptions specifically.
...
- **Solo variant**: common redirects include "this is bigger than I thought — let me brainstorm first" (suggest `/br-define`), "this is just a fix, no plan needed" (suggest `/br-work`), or "I need to investigate first" (suggest `/ce-debug`).
- **Brainstorm-sourced variant**: less common, but possible — "actually this scope is wrong, take it back to brainstorm" (suggest `/br-define` to revise the upstream doc).

In either case: stop br-plan, suggest the alternative skill, offer to load it in-session. Don't push back or argue — the user's redirect signal is the deliberate choice.
