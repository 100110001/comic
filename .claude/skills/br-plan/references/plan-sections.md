...
rendering reference (`markdown-rendering.md`).

## The outcome

A great plan enables three audiences to act:

- **The implementing agent** (`br-work` or a human) starts from an informed
...
Not every invocation of `br-plan` should produce a plan document. For
genuinely atomic work, the doc is ceremony — the implementer (whether
`br-work` or a human) can act directly without IDed units, KTDs, or
...
When skipping the plan doc, the work proceeds directly to `br-work` or to
implementation, and any decisions made along the way land in the commit
message, or folded into a spec at the harvest step if they're worth carrying forward.
...
- **Implementation Units** (with stable U-IDs) — the discrete units of work,
  sized so each is independently landable. `br-work` consumes these to
  execute. For trivial single-step plans the work may collapse into Summary
  prose and U-IDs may be omitted; this is rare.
- **Spec Impact** — which spec files the harvest step will create or update
  when the change is deemed complete (back-gate: specs are written at the END
  of the loop). For each: the spec path (existing `*.spec.md` /
  `specs/*.convention.md`, or a new one), whether it is **new** / **updated** /
  a **convention** note, and a one-line note on the design intent, invariant,
  or contract it will carry — goals-and-intent, not mechanics. This section is
  the forcing function that keeps spec reconciliation from being forgotten, so
  it is always present: a change that alters no design intent states
  `Spec Impact: none — no design intent changes.` Mechanics that won't matter
  post-hoc (intermediate fields, renames, migration choreography) do not belong
  here.
...
tracked work item. `br-work` does not mutate the plan at ship time;
...
- **`origin`** — repo-relative path to an upstream brainstorm requirements
  doc (e.g., `specs/changes/2026-05-12-pagination/define.md`).
  Set when planning from an upstream brainstorm; carried for traceability
  and re-resolved when `br-plan` re-deepens.
...
- **`execution`** — execution domain for downstream routing: `code`
  (the default when absent) or `knowledge-work`. `br-work`'s input triage
  ...
  to the normal code path. Written by `br-plan`'s approach-altitude flow
...
`references/markdown-rendering.md` describes how to render these sections.
This reference (`plan-sections.md`) is about WHAT the plan contains; the
rendering reference is about HOW it is presented.
