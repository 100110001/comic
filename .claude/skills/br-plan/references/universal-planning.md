...
This file is loaded when br-plan detects a non-software task (Phase 0.1b). It replaces the software-specific phases (0.2 through 5.1) with a domain-agnostic planning workflow.
...
- **Pipeline mode?** If invoked from LFG or any `disable-model-invocation` context: output "This is a non-software task. The LFG pipeline requires br-work, which only supports software tasks. Use `/br-plan` directly for non-software planning." and stop.
...
**Execution here is research and analysis only — never code.** Reading code and artifacts to understand them is in-bounds research; writing or running code to change the system is not — that belongs in `br-work`. This keeps the planning/execution boundary intact.
...
> Wrong: "Quick note first: /br-plan builds implementation plans, so I ignored the template and just answered the question. Here's what the data says..."
...
When research is recommended, do it — don't just offer. Stale recommendations (closed restaurants, rethemed attractions, outdated prices) are worse than no recommendations. The user invoked `/br-plan` because they want a good plan, not a disclaimer about training data.
...
1. **Save to disk** — Write the plan as a markdown file. Ask where:
   - `specs/changes/` (only show if this directory exists)
...
Do not offer `/br-work` (software-only) or issue creation (not applicable to non-software plans).
